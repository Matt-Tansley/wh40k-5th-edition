...
- One catalogue for each different army list (usually a Codex) - e.g `Space Marines.cat`
- Catalogues to represent expansions - e.g `The Badab War`. This is done for the sake of organisation and trying to declutter data files (for the The Badab War example, all the entries could have just gone in the Space Marine catalogue).
